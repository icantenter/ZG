...
exec="select * from shop_fl order by px_id asc" 
...
<form  name="add" method="post" action="shop_fl.asp?xiugaipx=ok">
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
        <tr>
          ...
          <td width="13%" bgcolor="#FFFFFF"><input type="button" name="Submit" value="删除分类" onClick="javascript:if(confirm('确定删除？删除后不可恢复!')){window.location.href='shop_fl.asp?act=del&id=<%=rs("id")%>';}else{history.go(0);}" /> </td>
        </tr>
      </table></form>
      <%rs.movenext
next
...
set rs=server.createobject("adodb.recordset")
sql="select title,etitle,px_id from shop_fl"
rs.open sql,conn,1,3
