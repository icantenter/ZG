...
exec="select * from bookshh where ssfl=3" 
...
<%For i=1 To x%>
      <table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#FFFFFF">
        <tr>
          ...
          <td width="11%" bgcolor="#FFFFFF"><input type="button" name="Submit" value="删除书籍" onClick="javascript:if(confirm('确定删除？删除后不可恢复!')){window.location.href='admin_bookshh3.asp?act=del&id=<%=rs("id")%>';}else{history.go(0);}" /> </td>
        </tr>
      </table>
      <%rs.movenext
next
