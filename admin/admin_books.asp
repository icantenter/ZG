...
<%For i=1 To x%>
      <table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#FFFFFF">
        <tr>
          ...
          <td width="13%" bgcolor="#FFFFFF"><input type="button" name="Submit3" value="修改书籍" onClick="window.location.href='xiugai_books.asp?id=<%=rs("id")%>' "/> </td>
          <td width="11%" bgcolor="#FFFFFF"><input type="button" name="Submit" value="删除书籍" onClick="javascript:if(confirm('确定删除？删除后不可恢复!')){window.location.href='admin_books.asp?act=del&id=<%=rs("id")%>';}else{history.go(0);}" /> </td>
        </tr>
      </table>
      <%rs.movenext
next
