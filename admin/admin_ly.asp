         ...
         <td width="13%">删除</td>
            ...
            <tr>
              <td width="8%" class="td"><input name="ID" type="checkbox" id="ID" value="<%=rs("id")%>" />
                <%=rs("id")%></td>
              <td width="22%" height="25" class="td"><a href="xiugai_ly.asp?id=<%=rs("id")%>" style="color:#003399"><%=left(rs("title"),20)%></a></td>
              <td width="10%" class="td"><%=rs("name")%></td>
              <td width="10%" class="td"><%=rs("useradmin")%></td>
              <td width="10%" class="td"><%
if IsNull(rs("hf")) or trim(rs("hf")&"")="" then
response.Write("<font color=#FF0000>[未回复]</font>")
else
response.Write("<font color=blue>[已回复]</font>")
end if
  %></td>
